...
\end{lem}
\begin{proof}
    $f=\1_{X},\ g=\1_{Y},\ h=\1_{Z}$とおけばよい．
\end{proof}
\begin{lem}\label{lem:bm1_implies_bm2}
    \eqref{eq:bm1}を認めれば\eqref{eq:bm2}は正しい．
\end{lem}
\begin{proof}
    $X$または$Y$が零集合なら自明なので，$\Lm^n(X)>0,\ \Lm^n(Y)>0$としてよい．適当にスケーリングすると，$Z\supset X+Y$のとき$\Lm^n(Z)^{1/n}\geq \Lm^n(X)^{1/n}+\Lm^n(Y)^{1/n}$であること示せばよいがわかる．
    \begin{equation}
        \lambda'\coloneqq \frac{\Lm^n(Y)^{1/n}}{\Lm^n(X)^{1/n}+\Lm^n(Y)^{1/n}},
    \end{equation}
    \begin{equation}
        X'\coloneqq \Lm^n(X)^{-1/n}X,\quad Y'\coloneqq \Lm^n(Y)^{-1/n}Y,\quad Z'\coloneqq \frac{Z}{\Lm^n(X)^{1/n}+\Lm^n(Y)^{1/n}}
    \end{equation}
    とおくと\eqref{eq:bm1}より$\Lm^n(Z)\geq1$を得る．左辺を変形すると\eqref{eq:bm2}を得る．
\end{proof}
...
\begin{proof}
    ...
    \begin{align}
        \int_{\Rset^n}f^{n/(n-1)}\,dx&=\frac{n}{n-1}\int_{0}^{\infty}t^{1/(n-1)}\Lm^n(\qty{f\geq t})\,dt\\
...
\begin{cor}\label{cor:marginal_concavity}
    \underline{連続な}非負値可積分関数$K=K(x,y)\colon \Rset^m\times \Rset^n\to [0,\infty)$は$\Rset^{m+n}$上$p$-concaveで$p\geq -1/n$とする\footnote{$K$の連続性の仮定は実は除ける．その際，$p$-concavityの定義を弱める必要がある．\ref{sec:ess_bbl}を参照．}．このとき
    \begin{equation}
...
\begin{cor}\label{cor:conv_concavity}
    ...
    $i=1,\,2$に対して\underline{連続な}非負値可積分関数$f_i\colon \Rset^n\to[0,\infty)$は$p_i$-concaveだとする\footnote{系\ref{cor:marginal_concavity}と同様に$f_i$の連続性は不要．}．このとき$f_1\ast f_2$は$p/(1+np)$-concaveである．
\end{cor}
\begin{proof}
...
\begin{cor}\label{cor:heat_concavity}
    \underline{連続な}非負値可積分関数$f\colon \Rset^n\to[0,\infty)$がlog-concaveだとする\footnote{$f$の連続性は不要．}．このとき，熱方程式の初期値問題
    \begin{equation}
        \left\{
        \begin{array}{rl}
...
\end{rmk}
\begin{rmk}
    Brascamp--Lieb \cite{BL}は，他にも\eqref{eq:pl}を用いることで，ポテンシャル$V$がconvexであるときにDirichlet境界条件を課した$\partial_t-\frac{1}{2}\Delta +V$の基本解がlog-concaveであることを示した．
\end{rmk}
\begin{rmk}
